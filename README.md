...
**Documentation** — all three are generated by `docs/build_docs.py`, so they
can be corrected and rebuilt rather than drifting out of date:

| Document | For |
|---|---|
| `docs/SecureLock_Explained.pdf` | Understanding the system from scratch, with no technical background |
| `docs/SecureLock_User_Manual.pdf` | Running the prototype and demonstrating every feature |
| `docs/SecureLock_Presentation_Guide.pdf` | Presenting at SIH round 1 — problem, existing systems, novelty, stack, advantages, limitations, and likely judge questions |

```bash
pip install reportlab && python docs/build_docs.py
```
